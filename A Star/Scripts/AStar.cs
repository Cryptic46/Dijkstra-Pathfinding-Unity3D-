...
public class AStar : MonoBehaviour
{
    ...
    private int HeuristicSearch(Vector2 goal, Vector2 nextNode)
    {
        return (int)((goal.x - nextNode.x) + (goal.y - nextNode.y));
    }
    ...
    private void DetermineCostOfNeighboringNodes()
    {
        ...
        for (int j = 0; j < neighborsOfCurrentNode.Count; ++j)
        {
            ...
            if (!costSoFarHash.Contains(neighborsOfCurrentNode[j]))
            {
                ...
                PrioritizeCostOfNewTile(HeuristicSearch(target.position, neighborsOfCurrentNode[j]), neighborsOfCurrentNode[j]);
